...
public class MathHelper
{
    ...
    static {
        for (int i = 0; i < BF_SIN_COUNT2; ++i) {
            BF_sinHalf[i] = (float)Math.sin((double)(((double)i + (double)Math.min((int)1, (int)(i % (BF_SIN_COUNT / 4))) * 0.5) / (double)BF_SIN_COUNT * (double)BF_radFull));
        }
    }
    /**
    ...
    public static float sin(float p_76126_0_)
    {
        ...
        return fastMath ? SIN_TABLE_FAST[(int)(p_76126_0_ * radToIndex) & 4095] : BF_sinHalf[index2] * (float)mul;
    }
    ...
    {
        return fastMath ? SIN_TABLE_FAST[(int)(value * radToIndex + 1024.0F) & 4095] : sin(value + BF_SIN_TO_COS);
    }
