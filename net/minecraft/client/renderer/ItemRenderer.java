...
import NovClient.Module.Modules.Render.Animations;
import NovClient.Module.Modules.Render.Animations.renderMode;
          ...
          if(this.itemToRender.getItem() == Items.filled_map) {
             ...
             case 4:
                 ...
                 if(!Client.instance.getModuleManager().getModuleByClass(Animations.class).isEnabled()) {
 					...
 					if(Ani.mode.getValue() == renderMode.Swang) {
 						var5 = 0;
 					}
 					if(Ani.mode.getValue() == renderMode.Swank) {
 						var5 = 1;
 					}
 					
 					if(Ani.mode.getValue() == renderMode.Swing) {
 						var5 = 2;
 					
 					}
 					if(Ani.mode.getValue() == renderMode.Swong) {
 						var5 = 3;
 					}
 					
 					if(Ani.mode.getValue() == renderMode.SwAing) {
 						var5 = 4;
 					}
 					if(Ani.mode.getValue() == renderMode.Remix) {
 						var5 = 5;
 					}
 					if(Ani.mode.getValue() == renderMode.Custom) {
 						var5 = 6;
 					}
 					if(Ani.mode.getValue() == renderMode.None) {
 						var5 = 7;
 					}
 					if(Ani.mode.getValue() == renderMode.Old) {
 						var5 = 8;
 					}
 					if(Ani.mode.getValue() == renderMode.Gay) {
 						var5 = 9;
 					}
 					if(Ani.mode.getValue() == renderMode.Punch) {
 						var5 = 10;
 					}
 					if(Ani.mode.getValue() == renderMode.Winter) {
 						var5 = 11;
 					}
 					if(Ani.mode.getValue() == renderMode.Random) {
 						var5 = 12;
 					}
             ...
             case 5:
                ...
                this.doBowTransformations(partialTicks, abstractclientplayer);
 				...
 				byte  var5 = 0;
 				Animations Ani = (Animations)Client.instance.getModuleManager().getModuleByClass(Animations.class);
         	if(hud.EveryThingBlock.getValue() && mc.gameSettings.keyBindUseItem.isKeyDown()) {
         		GL11.glTranslated(hud.x.getValue(),  hud.y.getValue(),  hud.z.getValue());
         		if(!Client.instance.getModuleManager().getModuleByClass(Animations.class).isEnabled()) {
 					this.transformFirstPersonItem(af / 2.0F, af1);
 	                this.doBlockTransformations();
 				}else {
...
//        float f1 = flag?0.0F:1.0F;
        ...
        float f22 = MathHelper.clamp_float(((KillAura.blockTarget !=null && KillAura.blockTarget.hurtTime > Animation.HurtTime.getValue().intValue()&&!Animation.Eliminates.getValue() || flag ) ? 0.0F : 1.0F) - this.equippedProgress, -f2, f2);
