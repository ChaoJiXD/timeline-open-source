...
import NovClient.Module.Modules.Render.FullBright;
import NovClient.Module.Modules.Render.NoHurtCam;
import NovClient.Module.Modules.Render.ViewClip;
...
public class EntityRenderer implements IResourceManagerReloadListener
{
    ...
    public void getMouseOver(float partialTicks)
    {
        ...
        {
        	Reach re = (Reach)Client.instance.getModuleManager().getModuleByClass(Reach.class);
            this.mc.mcProfiler.startSection("pick");
            this.mc.pointedEntity = null;
            double d0 = (re.isEnabled() && re.canReach((EntityLivingBase)this.pointedEntity))? re.BuildingReach.getValue() : (double)this.mc.playerController.getBlockReachDistance();
            this.mc.objectMouseOver = entity.rayTrace((re.isEnabled() && re.canReach((EntityLivingBase)this.pointedEntity))? re.CombatReach.getValue() : d0, partialTicks);
            double d1 = d0;
            ...
            if (re.isEnabled() && re.canReach((EntityLivingBase)this.pointedEntity)) {
            	d1 = re.CombatReach.getValue();
            ...
            if (this.pointedEntity != null && flag && vec3.distanceTo(vec33) > ((re.isEnabled() && re.canReach((EntityLivingBase)this.pointedEntity))? re.CombatReach.getValue() : 3.0D)  )
            {
                this.pointedEntity = null;
        ...
        if (this.lightmapUpdateNeeded)
        {
            ...
            if (world != null)
            {
                ...
                for (int i = 0; i < 256; ++i)
                {
                    ...
                    float f16 = (Client.instance.getModuleManager().getModuleByClass(Xray.class).isEnabled() || Client.instance.getModuleManager().getModuleByClass(FullBright.class).isEnabled()) ? 10000f : this.mc.gameSettings.gammaSetting;
                    float f17 = 1.0F - f8;
                    float f13 = 1.0F - f9;
                    float f14 = 1.0F - f10;
