...
public class MainPathFinder {
    private final Vec31 startVec3;
    private final Vec31 endVec3;
    private ArrayList<Vec31> path = new ArrayList();
    private final ArrayList<PathHub> pathHubs = new ArrayList();
    private final ArrayList<PathHub> workingPathHubList = new ArrayList();
    private static final Vec31[] directions = new Vec31[]{new Vec31(1.0, 0.0, 0.0), new Vec31(-1.0, 0.0, 0.0),
            new Vec31(0.0, 0.0, 1.0), new Vec31(0.0, 0.0, -1.0)};

    public MainPathFinder(final Vec31 startVec3, final Vec31 endVec3) {
        this.startVec3 = startVec3.addVector(0.0, 0.0, 0.0).floor();
        this.endVec3 = endVec3.addVector(0.0, 0.0, 0.0).floor();
    }

    public ArrayList<Vec31> getPath() {
        return this.path;
    }
    ...
    public void compute(final int loops, final int depth) {
        ...
        final ArrayList<Vec31> initPath = new ArrayList<>();
        ...
        for (int i = 0; i < loops; ++i) {
            ...
            for (final PathHub pathHub : new ArrayList<>(this.workingPathHubList)) {
                ...
                for (final Vec31 direction : directions) {
                    final Vec31 loc = pathHub.getLoc().add(direction).floor();
                    if (isValid(loc, false) && this.putHub(pathHub, loc, 0.0)) {
                        break block0;
                    }
                }

                final Vec31 loc1 = pathHub.getLoc().addVector(0.0, 1.0, 0.0).floor();
    ...
    public static boolean isValid(final Vec31 loc, final boolean checkGround) {
    ...
    public PathHub doesHubExistAt(final Vec31 loc) {
    ...
    public boolean putHub(final PathHub parent, final Vec31 loc, final double cost) {
        ...
        if (existingPathHub == null) {
            ...
            final ArrayList<Vec31> path = new ArrayList<>(parent.getPathway());
            path.add(loc);
    ...
    public static ArrayList<Vec31> computePath(Vec31 topFrom, final Vec31 to) {
        ...
        int i = 0;
        Vec31 lastLoc = null;
        Vec31 lastDashLoc = null;
        final ArrayList<Vec31> path = new ArrayList<>();
        final ArrayList<Vec31> pathFinderPath = pathfinder.getPath();

        for (final Vec31 pathElm : pathFinderPath) {
