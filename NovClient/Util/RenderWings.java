...
import NovClient.Module.Modules.Misc.Wings;
...
public class RenderWings extends  ModelBase {
   ...
   public RenderWings() {
      this.location = new ResourceLocation("NovAssets/ICON/wings.png");
   ...
   public void renderWings( float partialTicks) {
      ...
      double rotate = this.interpolate(per?mc.thePlayer.rotationYaw: ((Client.instance.getModuleManager().getModuleByClass(KillAura.class).isEnabled() && KillAura.blockTarget != null ||Client.instance.getModuleManager().getModuleByClass(Scaffold.class).isEnabled())?Client.Yaw: mc.thePlayer.prevRenderYawOffset), per?mc.thePlayer.rotationYaw:((Client.instance.getModuleManager().getModuleByClass(KillAura.class).isEnabled() && KillAura.blockTarget != null ||Client.instance.getModuleManager().getModuleByClass(Scaffold.class).isEnabled())||Client.instance.getModuleManager().getModuleByClass(Speed.class).isEnabled()?Client.Yaw: mc.thePlayer.renderYawOffset), partialTicks);
      GL11.glPushMatrix();
