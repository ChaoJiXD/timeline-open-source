...
public abstract class FontLoaders {
	...
	private static Font getComfortaa(int size) {
		...
		try {
			InputStream is = Minecraft.getMinecraft().getResourceManager()
					.getResource(new ResourceLocation("NovAssets/FONT/Comfortaa.ttf")).getInputStream();
	...
	private static Font getNovICON(int size) {
		...
		try {
			InputStream is = Minecraft.getMinecraft().getResourceManager()
					.getResource(new ResourceLocation("NovAssets/FONT/NovICON.ttf")).getInputStream();
			font = Font.createFont(0, is);
			font = font.deriveFont(0, size);
		} catch (Exception ex) {
			ex.printStackTrace();
